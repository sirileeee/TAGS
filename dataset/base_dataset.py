...
class BaseVolumeDataset_GTMask(Dataset):
    def __init__(
            self,
            image_paths,
            label_meta,
    ...
    def __getitem__(self, idx):
        ...
        if (np.max(img_spacing) / np.min(img_spacing) > 8) or (
            ...
            mask = (
                F.interpolate(
                    input=mask_tensor.unsqueeze(0).permute(0, 2, 1, 3, 4).contiguous(),
                    scale_factor=(img_spacing[0] / self.target_spacing[0], 1, 1),
                    mode="trilinear",
                )
                .squeeze(0)
            ...
            mask = (
                F.interpolate(
                    input=torch.tensor(mask[None, None, :, :, :]),
                    scale_factor=tuple(
                        [img_spacing[i] / self.target_spacing[i] for i in range(3)]
                    ),
                    mode="trilinear",
        ...
        if (self.aug and self.split == "train") or ((self.do_val_crop  and self.split=='val')):
            trans_dict = self.transforms({"image": img, "label": seg, "mask": mask})[0]
            img_aug, seg_aug, mask_aug = trans_dict["image"], trans_dict["label"], trans_dict["mask"]
        else:
            trans_dict = self.transforms({"image": img, "label": seg, "mask": mask})
            img_aug, seg_aug, mask_aug = trans_dict["image"], trans_dict["label"], trans_dict["mask"]
        seg_aug = seg_aug.squeeze()
        mask_aug = mask_aug.squeeze()

        img_aug = img_aug.repeat(2, 1, 1, 1)

        return img_aug, seg_aug, mask_aug, np.array(img_vol.header.get_zooms())[self.spatial_index]
    ...
    def get_transforms(self):
        ...
        if self.split == "train":
            ...
            transforms.extend(
                [
                    BinarizeLabeld(keys=["label", "mask"]),
                    SpatialPadd(
                        keys=["image", "label", "mask"],
                    ...
                    RandFlipd(keys=["image", "label", "mask"], prob=0.5, spatial_axis=0),
                    RandFlipd(keys=["image", "label", "mask"], prob=0.5, spatial_axis=1),
                    RandFlipd(keys=["image", "label", "mask"], prob=0.5, spatial_axis=2),
                    RandRotate90d(keys=["image", "label", "mask"], prob=0.5, max_k=3),
                ]
            )
...
class BaseVolumeDataset_Mask(Dataset):
    '''
    Compared to BaseVolumeDataset_GTMask, this class utilizes machine-generated organ masks in place of ground-truth organ masks.
    '''
    def __init__(
            self,
            image_paths,
