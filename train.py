from dataset.datasets_mask import load_data_volume
...
from modeling.image_encoder import ImageEncoderViT_3d
...
def main():
    ...
    parser.add_argument(
        "--data", default=None, type=str, choices=["kits", "pancreas", "lits"]
    )
    parser.add_argument(
        "--snapshot_path",
        default="",
    ...
    parser.add_argument("--num_classes", default=2, type=int, help="foreground (tumor) and background")
    ...
    parser.add_argument("--text_pretrained_weight", default="ViT-L/14", type=str, choices=["emilyalsentzer/Bio_ClinicalBERT", "ViT-L/14"])
    parser.add_argument("--use_mask", default=True)

    args = parser.parse_args()

    # args.snapshot_path = '/path/to/store/your/model'
    # args.data_prefix = ''
    # args.data = ''

    device = args.device
    if args.rand_crop_size == 0:
        if args.data in ["pancreas", "lits", "kits"]:
    ...
    sam = sam_model_registry["vit_b"](checkpoint="/checkpoint/path")
    ...
    for i in img_encoder.neck_3d:
        for p in i.parameters():
            p.requires_grad = True
    for i in img_encoder.alignment_adapters:
        for p in i.parameters():
            p.requires_grad = True
    ...
    text_encoder = TextEncoder(args.text_pretrained_weight, device=device)


    encoder_opt = AdamW([i for i in img_encoder.parameters() if i.requires_grad==True], lr=args.lr, weight_decay=1e-5)
    encoder_scheduler = torch.optim.lr_scheduler.LinearLR(encoder_opt, start_factor=1.0, end_factor=0.01, total_iters=500)
    feature_opt = AdamW(parameter_list, lr=args.lr, weight_decay=1e-5)
    feature_scheduler = torch.optim.lr_scheduler.LinearLR(feature_opt, start_factor=1.0, end_factor=0.01,
                                                          total_iters=500)
    decoder_opt = AdamW([i for i in mask_decoder.parameters() if i.requires_grad == True], lr=args.lr, weight_decay=1e-5)
    ...
    dice_loss1 = DiceLoss(include_background=False, softmax=False, to_onehot_y=True, reduction="none")
    ...
    with torch.amp.autocast('cuda'), torch.no_grad():
        text_features = text_encoder(args.data).to(torch.float32)

    for epoch_num in range(args.max_epoch):
        loss_summary = []
        alignment_loss_summary = []
        img_encoder.train()
        for module in prompt_encoder_list:
            module.train()
        mask_decoder.train()
        for idx, (img, seg, organ_mask, spacing) in enumerate(train_data):
            if args.use_mask:
                img = torch.cat((img, organ_mask.unsqueeze(0)), dim=1).to(device)

            out = F.interpolate(img.float(), scale_factor=512 / patch_size, mode='trilinear')
            input_batch = out.to(device)
            input_batch = input_batch[0].transpose(0, 1)
            batch_features, feature_list, align_feature_list = img_encoder(input_batch)
            ...
            mask = seg.squeeze(0).to(device)
            alignment_loss = 0
            for i, align_feature in enumerate(align_feature_list):
                align_feature = align_feature.permute(0, 4, 3, 1, 2)
                align_feature = align_feature.reshape(align_feature.shape[0], align_feature.shape[1], -1)
                align_feature = align_feature.permute(0, 2, 1)
                align_feature = align_feature / align_feature.norm(dim=-1, keepdim=True)

                seg_map = (100.0 * align_feature @ text_features)
                B, L, C = seg_map.shape
                H = int(np.cbrt(L))
                seg_map = F.interpolate(seg_map.permute(0, 2, 1).view(B, 2, H, H, H),
                                                    size=args.rand_crop_size[0], mode='trilinear', align_corners=True)
                seg_map = torch.softmax(seg_map, dim=1)

                alignment_loss += (focal_loss(seg_map, mask) + dice_loss1(seg_map[:, 1, :, :, :].unsqueeze(0), mask.unsqueeze(0).unsqueeze(0))) / 2
                if i == 3:
                    dice_feature = 1 - dice_loss1(seg_map[:, 1, :, :, :].unsqueeze(0), mask.unsqueeze(0).unsqueeze(0)).item()
                    logger.info(
                        'Dice Score for Stage {} is {}'.format(i, dice_feature)
                    )

            alignment_loss.requires_grad_(True)
            ...
            alignment_loss_summary.append(alignment_loss.detach().cpu().numpy())
            ...
            logger.info(
                'epoch: {}/{}, iter: {}/{}'.format(epoch_num, args.max_epoch, idx, len(train_data)) + ": loss:" + str(
                    loss_summary[-1].flatten()[0]) + "; alignment loss:" + str(alignment_loss_summary[-1].flatten()[0]))
        ...
        with torch.no_grad():
            loss_summary = []
            alignment_loss_summary = []
            for idx, (img, seg, organ_mask, spacing) in enumerate(val_data):
                ...
                input_batch = out.to(device)
                input_batch = input_batch[0].transpose(0, 1)
                batch_features, feature_list, alignment_feature_list = img_encoder(input_batch)
                feature_list.append(batch_features)
                l = len(torch.where(seg == 1)[0])
                points_torch = None
                if l > 0:
                ...
                mask = seg.squeeze(0).to(device)
                alignment_loss = 0
                for _, align_feature in enumerate(alignment_feature_list):
                    align_feature = align_feature.permute(0, 4, 3, 1, 2)
                    align_feature = align_feature.reshape(align_feature.shape[0], align_feature.shape[1], -1)
                    align_feature = align_feature.permute(0, 2, 1)
                    align_feature = align_feature / align_feature.norm(dim=-1, keepdim=True)
                    ...
                    seg_pred = seg_map[:, 1, :, :, :].unsqueeze(0)
                    alignment_loss += (focal_loss(seg_map, mask) + dice_loss1(seg_pred, mask.unsqueeze(0).unsqueeze(0))) / 2
                alignment_loss.requires_grad_(True)
                ...
                logger.info(
                    'epoch: {}/{}, iter: {}/{}'.format(epoch_num, args.max_epoch, idx, len(val_data)) + ": loss:" + str(
                        loss_summary[-1].flatten()[0]) + "; alignment loss:" + str(alignment_loss_summary[-1].flatten()[0]))
        logger.info("- Val metrics: " + str(np.mean(loss_summary)) + "; alignment loss:" + str(np.mean(alignment_loss_summary)))
