...
def main():
    ...
    def model_predict(img, prompt, img_encoder, prompt_encoder, mask_decoder):
        ...
        batch_features, feature_list, _ = img_encoder(input_batch)
    ...
    with torch.no_grad():
        loss_summary = []
        loss_nsd = []
        for idx, (img, seg, organ_mask, spacing) in enumerate(test_data):
            seg = seg.float()
            prompt = F.interpolate(seg[None, :, :, :, :], img.shape[2:], mode="nearest")[0]
            seg = seg.to(device).unsqueeze(0)
            if args.use_mask:
                img = torch.cat((img, organ_mask.unsqueeze(0)), dim=1)
            else:
                img = torch.cat((img, img[:, 0, :, :, :].unsqueeze(0)), dim=1)
            img = img.to(device)
