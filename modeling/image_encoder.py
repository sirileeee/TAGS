...
class AlignmentAdapter(nn.Module):
    def __init__(self, c_in, bottleneck=768):
        super(AlignmentAdapter, self).__init__()
...
class ImageEncoderViT_3d(nn.Module):
    def __init__(
        ...
        self.alignment_adapters = nn.ModuleList()
        for i in range(4):
            self.alignment_adapters.append(AlignmentAdapter(embed_dim, embed_dim))

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        with torch.no_grad():
            x = self.patch_embed(x)
        if self.num_slice > 1:
            # x.shape [D, H, W, C] -> [C, H, W, D] -> [B, C, H, W, D], B=1
            x = self.slice_embed(x.permute(3, 1, 2, 0).unsqueeze(0))
            # x.shape [B, C, H, W, D] -> [B, H, W, D, C]
        ...
        if self.pos_embed is not None:
            # pos_embed.shape [B, H, W, C] -> [B, C, H, W] --avg_pool2D--> [B, H//2, W//2, C] -> [B, H//2, W//2, 1, C]
            pos_embed = F.avg_pool2d(self.pos_embed.permute(0,3,1,2), kernel_size=2).permute(0,2,3,1).unsqueeze(3)
            # depth_embed.shape [B, D, C] -> [B, 1, 1, D, C]
            # pos_embed.shape [B, H//2, W//2, D, C]
        ...
        for blk in self.blocks[:6]:
            ...
            if idx % 3 == 0 and idx != 12:
                align_x = self.alignment_adapters[idx//3-1](x)
                x = 0.8 * x + 0.2 * align_x
                align_feature_list.append(align_x)
        ...
        align_x = self.alignment_adapters[-1](x)
        x = 0.8 * x + 0.2 * align_x
        align_feature_list.append(align_x)
        x = self.neck_3d[-1](x.permute(0, 4, 1, 2, 3))

        return x, feature_list, align_feature_list
